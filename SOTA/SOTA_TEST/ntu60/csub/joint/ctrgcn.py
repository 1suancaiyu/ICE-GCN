...
class TCE_k1(nn.Module):
    def __init__(self, in_channels, redu=16):
        super(TCE_k1, self).__init__()
        redu_channels = in_channels // redu

        self.spatial_pooling = nn.AdaptiveAvgPool2d((None,1))
        self.relu = nn.ReLU()
        self.conv_temporal = nn.Conv1d(in_channels=in_channels, out_channels=redu_channels, kernel_size=1, groups=1)
