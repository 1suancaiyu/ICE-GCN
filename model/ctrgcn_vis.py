...
def vis(original_tensor):
    print(">>>>>>>>>>> vis")
    import torchshow as ts
    ts.show(original_tensor)
    ts.save(original_tensor, './vis/test.jpg')  # You can specify the save path.
...
class SCE(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.unsqueeze(-2)
        sce_weights = self.sigmoid(x) # batch_size, channel, channel_weight=1, joints (n,c,1,25)

        # vis
        weights_vis = sce_weights.squeeze(2)[:1].squeeze(0) # 1, channel, joints
        # weights_vis_randn = weights_vis[torch.randperm(len(weights_vis))[:30], :]

        vis(weights_vis) # batch 1 c,v weights
