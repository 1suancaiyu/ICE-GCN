...
from model.ctrgcn import MultiScale_TemporalConv
...
class TCN_GCN_unit(nn.Module):
    def __init__(self, in_channels, out_channels, A, stride=1, residual=True, adaptive=True):
        ...
        self.tcn2 = MultiScale_TemporalConv(out_channels, out_channels, kernel_size=5, stride=stride, dilations=[1,2], residual=False)
        self.relu = nn.ReLU(inplace=True)
        if not residual:
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        self.l_low1 = TCN_GCN_unit(3, 256, A, stride=4, adaptive=adaptive, residual=True)
        # self.l_low7 = TCN_GCN_unit(128, 256, A, stride=2, adaptive=adaptive, residual=True)
        # self.l_low8 = TCN_GCN_unit(256, 256, A, adaptive=adaptive)
        self.l_low9 = TCN_GCN_unit(256, 256, A, adaptive=adaptive)
    ...
    def forward(self, x):
        ...
        # x_low8 = self.l_low8(x)

        x = self.l9(x)

        # x_low9 = self.l_low9(x)

        x = self.l10(x)

        x = torch.cat((x_low1, x_low9, x), dim=1)
