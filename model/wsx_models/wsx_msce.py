...
import sys
sys.path.append('')
from ..wsx_net_blocks.multi_scale_channel_excitation import Temporal_Channel_Excitation_conv1d_two_layer as TCE
from ..wsx_net_blocks.multi_scale_channel_excitation import Spatial_Channel_Excitation as SCE
...
class unit_tcn(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=5, stride=1):
        ...
        conv_init(self.conv)
        bn_init(self.bn, 1)
        self.tce = TCE(out_channels, out_channels, stride=stride)
        self.sce = SCE()

    def forward(self, x):
        x = self.bn( self.tce(self.sce( self.conv(x) )) )
        return x
