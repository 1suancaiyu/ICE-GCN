...
import sys
sys.path.append('')
from ..wsx_net_blocks.multi_scale_temporal_excitation import multi_scale_temporal_excitation as mste
...
class unit_tcn(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=5, stride=1):
        ...
        conv_init(self.conv)
        bn_init(self.bn, 1)
        self.mste = mste(out_channels)

    def forward(self, x):
        x = self.bn( self.mste( self.conv(x) ) )
        return x
