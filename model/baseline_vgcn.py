...
class unit_gcn(nn.Module):
    def __init__(self, in_channels, out_channels, A, adaptive=True):
        ...
        self.num_point = A.shape[2]
        self.adaptive = adaptive

        if adaptive:
            Bk = np.stack([np.eye(self.num_point)] * self.num_point, axis=0)
            self.Bk = nn.Parameter(torch.from_numpy(Bk.astype(np.float32)), requires_grad=True)
        ...
        for i in range(self.num_point):
            conv_branch_init(self.conv_d[i], self.num_point)
